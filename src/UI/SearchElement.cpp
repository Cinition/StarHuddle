...
void SearchElement::drawFocused( Vector2 _cursor_position )
{
	...
	RaylibDrawText( m_input.c_str(), static_cast< int >( text_pos.x ), static_cast< int >( text_pos.y ), font_size, UI::TEXT_COLOR );
	// draw blinker
}
...
void SearchElement::drawNotFocused( Vector2 _cursor_position )
{
	...
	auto    font_size = 16;
	Vector2 text_pos  = Vector2( _cursor_position.x + ( UI::MARGIN / 2 ), _cursor_position.y + ( UI::MARGIN / 2 ) + ( 16 / 2 ) );
	RaylibDrawText( "Search for any Asset", static_cast< int >( text_pos.x ), static_cast< int >( text_pos.y ), font_size, UI::TEXT_COLOR );
}
